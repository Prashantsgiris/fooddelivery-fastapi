...
from food import schemas, database,models, token
...
def login_as_owner(request:schemas.loginowner, db: Session = Depends(get_db)):
...
def login_as_Customer(request:schemas.login, db:Session = Depends(get_db)):
    user = db.query(models.logincustomer).filter(models.logincustomer.email == request.email).first()
    if not user:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail=f"Invalid Credentials")
    if not Hash.verify(user.password,request.password):
...
def login_as_delivery(request:schemas.login,db:Session=Depends(get_db)):
